...
update teacher set Faculty = 'ФИТ';
update teacher set Faculty = 'ИдиП' 
where Teacher_name='Насковец Михаил Трофимович';
commit;
...
select
 ...
 WHERE to_char(birthday,'Day')='Понедельник';
...
avg3 teacher.salary%type;
 begin
  select avg(salary) into avg1 from teacher where Pulpit ='ИСиТ';
select avg(salary)into avg2 from teacher where Pulpit ='ОВ';
select avg(salary)into avg3 from teacher where Pulpit ='ТЛ';
dbms_output.put_line('средняя зарплата на кафедре ИСиТ = '||avg1);
dbms_output.put_line('средняя зарплата на кафедре OB = '||avg2);
dbms_output.put_line('средняя зарплата на кафедре ТЛ = '||avg3);
end;
...
begin
new_row.teacher :='ВЛДМ';
new_row.teacher_name:='Вольский Дмитрий ';
new_row.pulpit:='ИСиТ';
